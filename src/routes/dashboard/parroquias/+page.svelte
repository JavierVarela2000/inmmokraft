...
<Modal title="¡Alerta!" bind:open={showDeleteAlert} autoclose outsideclose>
	<p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
		Estas por eliminar una parroquia
	</p>
	<svelte:fragment slot="footer">
		<form
			use:enhance={() => {
...
<main class="py-10">
	<div class="w-[95%] max-w-1440px mx-auto flex justify-between">
		<a class=" bg-blue-dark text-white p-4" href="/dashboard/parroquias/insertar"
			><span class="text-xl">+</span> Agregar Parroquia</a
		>
	...
	{:else}
		<Table class="w-[95%] max-w-1440px mx-auto shadow my-10" hoverable={true}>
			...
			<TableBody class="divide-y">
				{#each parroquias as parroquia}
					<TableBodyRow>
						<TableBodyCell>{parroquia.id}</TableBodyCell>
						<TableBodyCell>{parroquia.nombre}</TableBodyCell>
						<TableBodyCell>{parroquia.latitud}</TableBodyCell>
						<TableBodyCell>{parroquia.longitud}</TableBodyCell>
						<TableBodyCell tdClass="flex justify-center items-center gap-1">
							<Button
								href="/dashboard/parroquias/{parroquia.id}"
								class="font-medium  hover:underline ">Edit</Button
							>
							<Button
								color="red"
								on:click={() => {
